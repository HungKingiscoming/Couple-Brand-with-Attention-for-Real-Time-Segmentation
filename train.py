# ============================================
# train.py — GCNet v3 + GCNetHead v2 (fixed gradient explosion)
# ============================================
...
# ============================================
# PRETRAINED WEIGHT LOADER (giữ nguyên)
# ============================================
...
# ============================================
# OPTIMIZER (fixed với head_lr_factor)
# ============================================
def build_optimizer(model, args):
    ...
    groups = []
    if head_params:
        head_lr = args.lr * getattr(args, 'head_lr_factor', 0.5)
...
def diagnose_gradients(model, step, writer=None, threshold=10.0):
    """In chi tiết gradient norms."""
    grad_info = {}
    total_norm = 0.0
    ...
    sorted_grads = sorted(grad_info.items(), key=lambda x: x[1], reverse=True)
    if total_norm > threshold or len(sorted_grads) > 0:
        print(f"\n[Step {step}] Total grad norm: {total_norm:.4f}")
        if sorted_grads:
            print("Top 8 largest gradients:")
            for name, norm in sorted_grads[:8]:
                print(f"  {name[:70]}: {norm:.4f}")
    if writer:
        writer.add_scalar('debug/total_grad_norm', total_norm, step)
        for name, norm in sorted_grads[:5]:
            safe_name = name.replace('.', '_')
            writer.add_scalar(f'debug/grad_{safe_name}', norm, step)
    return total_norm, sorted_grads
        ...
        if hasattr(model.backbone, stage_name):
            ...
            if base_mod is not None and parts[1].isdigit():
                try:
                    ...
                    pass
        if module is None:
            print(f"  [skip] module '{stage_name}' not found")
            continue
        count = 0; bn_count = 0
        for p in module.parameters():
...
class Trainer:
    def __init__(self, model, optimizer, scheduler, device, args, class_weights=None):
        ...
        # Rescale class weights if too large
        if class_weights is not None:
            if class_weights.max() > 10:
                print(f"⚠️ Rescaling class weights from max={class_weights.max():.2f} to 5.0")
    ...
    def _print_config(self, loss_cfg):
        ...
        print(f"Head warmup epochs:      {self.head_warmup_epochs}")
    ...
    def train_epoch(self, loader, epoch):
        ...
        # Head warmup: freeze everything except head
        if epoch < self.head_warmup_epochs:
            if not self._head_warmup_active:
                print(f"\n>>> Head warmup mode (epoch {epoch+1}/{self.head_warmup_epochs}) <<<")
            ...
            # Also force aux_weight = 0 during warmup
            effective_aux_weight = 0.0
        else:
            if self._head_warmup_active:
                print("\n>>> Head warmup finished, restoring original freeze config <<<")
                # Restore freeze settings based on args
                if self.args.freeze_backbone:
                    ...
                    for param in self.model.backbone.parameters():
                        param.requires_grad = True
                self._head_warmup_active = False
            effective_aux_weight = self.args.aux_weight if not getattr(self.args, 'disable_aux', False) else 0.0
        ...
        for batch_idx, (imgs, masks) in enumerate(pbar):
            ...
            with autocast(device_type='cuda', enabled=self.args.use_amp):
                ...
                # Aux loss (only if not warmup and not disabled)
                if effective_aux_weight > 0:
                    aux_weight = effective_aux_weight * (1 - epoch / self.args.epochs) ** 0.9
            ...
            if (batch_idx + 1) % self.args.accumulation_steps == 0:
                ...
                # Gradient clipping with separate thresholds for head
                if self.args.grad_clip > 0:
                    head_params = [p for n, p in self.model.named_parameters() if 'decode_head' in n]
                    other_params = [p for n, p in self.model.named_parameters() if 'decode_head' not in n]
                    if head_params:
                        torch.nn.utils.clip_grad_norm_(head_params, self.args.grad_clip * 0.5)  # stricter
                    if other_params:
                        torch.nn.utils.clip_grad_norm_(other_params, self.args.grad_clip)

                # Optional detailed gradient logging (every 50 steps)
                if batch_idx % 50 == 0:
                    total_norm, _ = diagnose_gradients(self.model, self.global_step, self.writer, threshold=5.0)
        ...
        n = len(loader)
        print(f"\nEpoch {epoch+1} — Max gradient norm: {max_grad_epoch:.2f}")
        torch.cuda.empty_cache()
        if self.scheduler and self.args.scheduler != 'onecycle':
            self.scheduler.step()
...
def main():
    ...
    parser.add_argument("--head_lr_factor",         type=float, default=0.5, help="LR factor for head")
    parser.add_argument("--use_class_weights",      action="store_true")
    parser.add_argument("--head_warmup_epochs",     type=int,   default=2, help="Train only head first N epochs")
    parser.add_argument("--disable_aux",            action="store_true", default=False, help="Disable auxiliary loss")
    ...
    parser.add_argument("--debug_activations", action="store_true", help="Print activation stats")
    ...
    print(f"Head warmup: {args.head_warmup_epochs} epochs")
    print(f"{'='*70}\n")
    ...
    # ---- CRITICAL: Reinitialize head with very small weights ---- #
    print("Reinitializing head with small gain (a=0.1) to prevent gradient explosion...")
    def small_init_head(m):
        if isinstance(m, nn.Conv2d):
            ...
            nn.init.constant_(m.bias, 0)
    model.decode_head.apply(small_init_head)
    # Also reinit auxiliary head specifically
    if hasattr(model.decode_head, 'aux_head_c4'):
        model.decode_head.aux_head_c4.apply(small_init_head)
    if hasattr(model.decode_head, 'aux_cls_seg_c4'):
        nn.init.kaiming_normal_(model.decode_head.aux_cls_seg_c4.weight, mode='fan_out', nonlinearity='relu', a=0.1)
        if model.decode_head.aux_cls_seg_c4.bias is not None:
            nn.init.constant_(model.decode_head.aux_cls_seg_c4.bias, 0)

    print("Applying init_weights (backbone only)...")
    model.apply(init_weights)  # this will reinit backbone too, but pretrained will overwrite
    ...
    with torch.no_grad():
        ...
        try:
            ...
            print(f"  c4_logit: {c4_logit.shape}  mean={c4_logit.mean().item():.4f} std={c4_logit.std().item():.4f}")
            print(f"  c6_logit: {c6_logit.shape}  mean={c6_logit.mean().item():.4f} std={c6_logit.std().item():.4f}\n")
            if args.debug_activations:
                # Register forward hooks
                def hook_fn(name, module):
                    def hook(module, inp, out):
                        if isinstance(out, torch.Tensor):
                            print(f"[{name}] out: mean={out.mean().item():.4f} std={out.std().item():.4f} max={out.max().item():.4f}")
                    return hook
                for n, m in model.named_modules():
                    if 'decode_head' in n and ('conv' in n or 'cls_seg' in n):
                        m.register_forward_hook(hook_fn(n, m))
    ...
    UNFREEZE_STAGES = [
        ...
        ['semantic_branch_layers.0', 'detail_branch_layers.0', 'dwsa_stage4', 'compression_1', 'down_1'],
    ...
    for epoch in range(trainer.start_epoch, args.epochs):
        ...
        # Rebuild optimizer + scheduler when unfreeze
        ...
        # Switch back to full loss after ce_only phase
        if unfreeze_epochs:
